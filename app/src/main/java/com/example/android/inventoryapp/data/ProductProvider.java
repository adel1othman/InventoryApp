...
public class ProductProvider extends ContentProvider {
    ...
    private Uri insertProduct(Uri uri, ContentValues values) {
        ...
        if (title == null) {
            throw new IllegalArgumentException("You missed product title");
        }

        double price = values.getAsDouble(ProductEntry.COLUMN_PRODUCT_PRICE);
        if (String.valueOf(price).isEmpty() || price < 0) {
            throw new IllegalArgumentException("Product requires price value and it can\'t be negative");
        }
    ...
    private int updateProduct(Uri uri, ContentValues values, String selection, String[] selectionArgs) {
        ...
        if (values.containsKey(ProductEntry.COLUMN_PRODUCT_PRICE)) {
            double price = values.getAsDouble(ProductEntry.COLUMN_PRODUCT_PRICE);
            if (!ProductEntry.isValidInfo(String.valueOf(price)) || price < 0) {
                values.put(ProductEntry.COLUMN_PRODUCT_PRICE, 0);
                //Toast.makeText(getContext(), "Product requires price value and it can\'t be negative", Toast.LENGTH_SHORT).show();
            }
        }
        ...
        if (values.containsKey(ProductEntry.COLUMN_PRODUCT_QUANTITY)) {
            ...
            if (quantity < 0){
                ...
                //Toast.makeText(getContext(), "Quantity value can\'t be negative", Toast.LENGTH_SHORT).show();
            }
        }
